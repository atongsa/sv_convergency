...
Output is a matrix. Each row is an dome accession, and each column is the non-overlapping window. Each element is
the maximum Jaccard similarity of SVs in that window between that accession and the pimp accessions.
"""

parser = argparse.ArgumentParser(description='Get Jaccard similarity between dome and a comparison group.')
...
parser.add_argument("species_file", metavar="<group.txt>", type=str, help="First column is the phylogenetic group (dome/wild), second column is the accession")
parser.add_argument("species", metavar="<WILD>", type=str, default="WILD", help="Group to compare to: wild")
...
parser.add_argument("-o", metavar="<simi_sp.tsv>", type=str, help="simi_sp_tsv")
...
distances = np.zeros((len(dome), n_windows))
        ...
        if line.startswith("#"):
            ...
            if (at_chr == chr_used):
                ...
                if widx == current_window:
                    ...
                    # Iterate over the domes
                    t_distances = [[] for i in range(len(dome))]
                    ...
                    # Get the max % shared SVs between a given dome and each comp species sample
